...
void compileAndCheck(GLuint shader)
{
    ...
    if (status == GL_FALSE) {
        ...
        gl_GetShaderInfoLog (shader, infoLogLength, nullptr, infoLog);
...
GLuint compileShaderSource(GLenum type, GLsizei count, const ourGLchar **string)
{
    ...
    gl_ShaderSource (shader, count, string, nullptr);
...
void linkAndCheck(GLuint program)
{
    ...
    if (status == GL_FALSE) {
        ...
        infoLog = (ourGLchar*) malloc (infoLogLength);
        gl_GetProgramInfoLog (program, infoLogLength, nullptr, infoLog);
...
static void drawSolidCube()                { glutSolidCube(orad);                            }  /* orad doubles as size input */
static void drawWireCube()                 { glutWireCube(orad);                             }  /* orad doubles as size input */
...
static void drawSolidRhombicDodecahedron() { glutSolidRhombicDodecahedron ();                }
static void drawWireRhombicDodecahedron()  { glutWireRhombicDodecahedron ();                 }
static void drawSolidIcosahedron()         { glutSolidIcosahedron ();                        }
static void drawWireIcosahedron()          { glutWireIcosahedron ();                         }
static void drawSolidSierpinskiSponge()    { glutSolidSierpinskiSponge (depth, offset, orad);}  /* orad doubles as size input */
static void drawWireSierpinskiSponge()     { glutWireSierpinskiSponge (depth, offset, orad); }  /* orad doubles as size input */
static void drawSolidTorus()               { glutSolidTorus(irad,orad,slices,stacks);        }
static void drawWireTorus()                { glutWireTorus (irad,orad,slices,stacks);        }
static void drawSolidSphere()              { glutSolidSphere(orad,slices,stacks);            }  /* orad doubles as size input */
static void drawWireSphere()               { glutWireSphere(orad,slices,stacks);             }  /* orad doubles as size input */
static void drawSolidCone()                { glutSolidCone(irad,orad,slices,stacks);         }  /* irad doubles as base input, and orad as height input */
static void drawWireCone()                 { glutWireCone(irad,orad,slices,stacks);          }  /* irad doubles as base input, and orad as height input */
static void drawSolidCylinder()            { glutSolidCylinder(irad,orad,slices,stacks);     }  /* irad doubles as radius input, and orad as height input */
static void drawWireCylinder()             { glutWireCylinder(irad,orad,slices,stacks);      }  /* irad doubles as radius input, and orad as height input */
...
static void drawSolidCuboctahedron()
{
...
static void drawWireCuboctahedron()
{
  GLfloat radius = RADIUSFAC*(GLfloat)orad; /* orad doubles as size */
...
static void DrawSizeInfo(int *row)
{
    ...
    case GEO_RAD_HEIGHT:
        ...
        break;
    default:
        shapesPrintf ((*row)++, 1, "Unknown drawSizeInfoFlag: %d", table [function_index].drawSizeInfoFlag);
        break;
    }
}
